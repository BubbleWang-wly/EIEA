...
class MMEaDtaset():
    def __init__(self, file_path, device, ratio=0.2):
        self.device = device
        self.kg1 = read_tri_list(file_path + '/triples_1_')
        self.kg2 = read_tri_list(file_path + '/triples_2_')

        self.train_pair = self.load_alignment_pair(file_path + '/sup_ent_ids')
        self.test_pair = self.load_alignment_pair(file_path + '/dev_ent_ids')

        self.train_pair = np.array(self.train_pair)
        self.test_pair = np.array(self.test_pair)

        self.rest_pair = self.test_pair.copy().T
        self.rest_set_1, self.rest_set_2 = self.rest_pair
        self.rest_set_1 = list(self.rest_set_1)
        self.rest_set_2 = list(self.rest_set_2)


    def assign_dataset(self, all_pair, ratio):
        len1 = int(len(all_pair) * ratio)
        train_pair = all_pair[:len1]
        test_pair = all_pair[len1:]
    ...
    def reconstruct_search(self, gid1, gid2, tri1, tri2, new=False, entity_num=0, rel_num=0):
        ...
        for head, r, tail in old_triples:
            ...
            triples.append([int(head), r + 1, int(tail)])
        # print(triples)
        entity_num = len(entity)
        rel_num = len(rel)

        adj_matrix, r_index, r_val, \
            adj_features, rel_features, adj = self.get_matrix(triples,
                                                         entity,
        ...
        return ent_adj, rel_adj, node_size, rel_size, adj_list, r_index, r_val, triple_size, adj
    ...
    def get_matrix(self, triples, entity, rel, new=False, entity_num=0, rel_num=0):
        ...
        return adj_matrix, np.array(r_index), np.array(r_val), adj_features, rel_features, adj
...
class DBpDataset:
    ...
    def read_rel_fact_pair(self, path):
        ...
        with open(path + '/rel_fact_pair_id') as f:
            ...
            for cur in lines:
                cur = cur.strip().split('\t')
                rel_fact[((int(cur[0]), int(cur[1]), int(cur[2])), (int(cur[3]), int(cur[4]), int(cur[5])))] = float(
                    cur[6])
        return rel_fact
    ...
    def reconstruct(self, gid1, gid2, new=False):
        ...
        for i in range(r_index.shape[1]):
            ...
            if r_index[1][i] > max(self.rel):
                fact_confi[i] = self.rel_fact[
                    (int(ent_adj[1][index]), int(r_index[1][i]) - max(self.rel) - 3, int(ent_adj[0][index]))]
        ...
        # print(r_val)
        return [int(gid1)], [
            int(gid2)], ent_adj, rel_adj, node_size, rel_size, adj_list, r_index, r_val, triple_size, fact_confi, tri_list, tri1, tri2, d_reverse

    def reconstruct_search(self, gid1, gid2, tri1, tri2, new=False, entity_num=0, rel_num=0):
        # reconstruct_search的主要作用是根据给定的三元组(头实体、关系、尾实体)
        # 重构邻接矩阵和其他相关数据结构, 为后续的知识图谱表示学习做准备。
        entity = set()
        old_triples = tri1 + tri2
        ...
        # print(len(entity))  # 38960
        # print('max '+str(max(entity)))
        # print('max rel ' + str(max(rel)))
        adj_matrix, r_index, r_val, \
            adj_features, rel_features = self.get_matrix(triples,
                                                         entity,
                                                         rel, new, entity_num, rel_num)
        m_adj = None
        # print(rel_features)

...
def sinkhorn_test_3(scores, len_pair, adj, val_pair, device):
    ...
    # l = 1
    # scores = new_adj_1 * scores * new_adj_2

    # l=0 + l=1
    # scores = scores + new_adj_1 * scores * new_adj_2
    # scores = scores + new_adj_2 * scores * new_adj_1
...
class RUN():
    ...
    def load_dataset(self):
        ...
        # if self.args.ratio == 0.5:
        #     self.G_dataset = MMEaDtaset('../data/MMEA-data/seed0.5/{}'.format(DATASET_NAME),
        #                                 device=self.device, ratio=self.args.ratio)
        # elif self.args.ratio == 0.8:
        #     self.G_dataset = MMEaDtaset('../data/MMEA-data/seed0.8/{}'.format(DATASET_NAME),
        #                                 device=self.device, ratio=self.args.ratio)
        # else:
        #     self.G_dataset = MMEaDtaset('../data/MMEA-data/{}'.format(DATASET_NAME),
        #                                 device=self.device, ratio=self.args.ratio)
        ...
        if dataset == 'FB_DB':
            ...
            # else:
            #     datadir = '../data/MSP_results/FB15K-YAGO15K/'
        ...
        for filename in os.listdir(datadir):
            ...
            if filename.endswith('.npy'):
                if filename.endswith('name.npy'):
                    ...
                    print('1.2. loading side modality...')
                    print(filename)
                    print(moda_np.shape)
                    # sinkhorn_test_3(moda_np, moda_np.shape[0], self.adj, self.dev_pair, device=self.device)
                else:
                    print('1.2. loading side modality...')
                    moda_np = np.load(os.path.join(datadir, filename))
                    assert moda_np.max() < 1.5
                    self.side_modalities[filename.split('.')[0]] = moda_np

                    print(filename)
                    print(moda_np.shape)
                #     sinkhorn_test_3(moda_np, moda_np.shape[0], self.adj, self.dev_pair, device=self.device)
                # all_score = all_score + moda_np
        time2 = time.time()
        ...
        for epoch in range(train_epoch):
            ...
            if epoch >= 9 and epoch % 5 == 4 and self.semi_pred:
                ...
                with torch.no_grad():
                    ...
                    new_pair_1 = self.pred_pair_confidence(scores)
                    ...
                    for i, (_, side_score) in enumerate(self.side_modalities.items()):
                        ...
                        # scores = scores * 0.5 + torch.Tensor(side_score[rows_to_keep, cols_to_keep]) * self.side_weight * 0.5
                        # if i == 1:
                        #     break
                        if i == 0:
                            # new_pair_2 = self.pred_pair_confidence(scores)
                            new_pair_2 = self.pred_pair_confidence(scores / float(i + 2))
                        elif i == 1:
                            new_pair_3 = self.pred_pair_confidence(scores / float(i + 2))
                        else:
                            new_pair_4 = self.pred_pair_confidence(scores / float(i + 2))
                    ...
                    # semantic-branch的study
                    # new_pair_s = self.pred_pair_s(scores_s)
                    # new_pair_s = self.pred_pair_confidence(scores_s)
                    ...
                    new_pair = new_pair_1 | new_pair_2 | new_pair_3 | new_pair_s | new_pair_4
